...
# Identify root directory to allow for relative pathing
root <- "C:/Users/joryh/ownCloud/documents/Tufts/Hydro_Alteration/HA_HypTest/Codes"

# Source FDC-annual function for generating annual flow duration curves
source(paste0(root,"/FDC_annual.R"))
source(paste0(root,"/FDC_overall.R"))

# Reference pre-dam and post-dam daily flow time series
Q_out_res <- load("Q_out_results.Rdata")
...
# Subset param_set to choose parameter combinations to run from HP_Res_Simulation
params <- read.csv("params.csv")
param_set <- params

# param_set <- params[params[,1]==32,] #Use parameter combination 32
